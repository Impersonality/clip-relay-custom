...
async fn main() -> anyhow::Result<()> {
    ...
    let protected = Router::new()
        ...
        .route(
            "/clipboard/:id",
            get(get_clipboard).put(update_clipboard).delete(delete_clipboard),
        )
        .route(
            "/clipboard/:id/share",
    ...
    let app =
        Router::new()
            ...
            // 处理 /s/{id} 格式的分享链接，重定向到 /s/?token={id}
            .route(
                "/s/:id",
                get(|Path(id): Path<String>| async move {
                    Redirect::permanent(&format!("/s/?token={}", id))
                }),
            )
            // Root path
            .route(
...
async fn create_clipboard(
    ...
    let mut custom_suffix: Option<String> = None; // 自定义链接尾缀
    ...
    // 根据是否有custom_suffix生成或使用ID，并验证唯一性
    let id = if let Some(suffix) = custom_suffix {
        // 检查数据库中是否已存在该ID
        let conn = state.db.lock().unwrap();
        let exists: bool = conn
            .query_row(
                "SELECT COUNT(*) FROM ClipboardItem WHERE id = ?",
                [&suffix],
                |r| r.get::<_, i64>(0).map(|n| n > 0),
            )
            .unwrap_or(false);
        drop(conn); // 显式释放锁，避免后续死锁
        
        if exists {
            return (
                StatusCode::CONFLICT,
                Json(serde_json::json!({"error": "此链接后缀已被使用，请选择其他后缀"})),
            );
        }
    ...
    let new_weight: i64 = {
        ...
        if let Err(e) = conn.execute(
            ...
            return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error":"db write failed","detail": e.to_string()})));
        }
        w
    };
...
async fn update_clipboard(
    State(state): State<AppState>,
    Path(id): Path<String>,
    Json(req): Json<UpdateClipboardReq>,
) -> impl IntoResponse {
    let now = now_unix();
    let conn = state.db.lock().unwrap();
    ...
    let result = conn.execute(
        "UPDATE ClipboardItem SET content=?, updatedAt=? WHERE id=?",
        params![req.content, now, id],
    );
    
    if result.is_err() || result.unwrap() == 0 {
        return (
            StatusCode::NOT_FOUND,
            Json(serde_json::json!({"error": "Clipboard item not found"})),
        )
            .into_response();
    }
    ...
    let row = conn.query_row(
        "SELECT id,type,content,fileName,fileSize,sortWeight,contentType,inlineData,filePath,createdAt,updatedAt FROM ClipboardItem WHERE id=? LIMIT 1",
        [id.clone()],
        |r| {
            Ok(ClipboardItem {
                id: r.get(0)?,
    ...
    match row {
        ...
        Err(_) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({"error": "Failed to fetch updated item"})),
        )
            .into_response(),
    }
}
...
async fn get_item_share(
    ...
    if invalid {
        // Auto provision - 使用 id 作为 token，保持格式一致
        let new_token = id.clone();
        let conn = state.db.lock().unwrap();
        let _ = conn.execute(
...
async fn update_item_share(
    ...
    if matches!(req.reset, Some(true)) {
        ...
        token = new_token;
    } else if req.password.is_some() || req.expires_in.is_some() || req.max_downloads.is_some() || req.custom_suffix.is_some() {
            // 处理自定义尾缀修改
            if let Some(new_suffix) = req.custom_suffix.as_ref() {
                let new_suffix_trimmed = new_suffix.trim();
                if !new_suffix_trimmed.is_empty() && new_suffix_trimmed != token {
                    // 检查新尾缀是否已被占用（检查 ShareLink.token）
                    let token_exists: bool = conn
                        .query_row(
                            "SELECT COUNT(*) FROM ShareLink WHERE token = ?",
                            [new_suffix_trimmed],
                            |r| r.get::<_, i64>(0).map(|n| n > 0),
                        )
                        .unwrap_or(false);
                    ...
                    // 只更新 ShareLink 的 token，不修改 ClipboardItem.id
                    let _ = conn.execute(
                        "UPDATE ShareLink SET token=?, updatedAt=? WHERE token=?",
                        params![new_suffix_trimmed, now, token],
                    );
                    
                    token = new_suffix_trimmed.to_string();
                }
            }
