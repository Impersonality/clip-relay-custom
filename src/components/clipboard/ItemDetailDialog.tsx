...
export default function ItemDetailDialog({
  ...
  onRequestEdit?: (id: string, content: string) => Promise<void>;
}) {
  ...
  const handleSaveEdit = async () => {
    if (!onRequestEdit || !item) return;
    try {
      setIsSaving(true);
      await onRequestEdit(item.id, editContent);
      setIsEditing(false);
      toast({ title: "已保存", description: "内容已成功更新" });
    } catch (e: any) {
      toast({
        title: "保存失败",
    ...
    try {
      ...
      if (ok) {
        ...
        toast({
          title: "无法复制图片",
          description:
            "当前使用非安全连接（HTTP），浏览器不支持图片复制。请在 HTTPS 环境下尝试。",
          variant: "destructive",
        });
      }
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl md:max-w-4xl lg:max-w-5xl max-h-[85vh] overflow-y-auto custom-scrollbar w-[calc(100vw-2rem)] sm:w-auto">
        <DialogHeader>
              ...
              <div className="relative bg-muted p-4 rounded-lg group">
                ...
                {isEditing ? (
                  <Textarea
                    ...
                    className="min-h-[200px] text-sm resize-y border-0 bg-transparent p-0 focus-visible:ring-0 focus-visible:ring-offset-0"
                    placeholder="输入内容..."
                  />
                ) : (
