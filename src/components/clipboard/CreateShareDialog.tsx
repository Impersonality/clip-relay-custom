...
export default function CreateShareDialog({
  ...
  useEffect(() => {
    /* initialTab ignored in settings-only mode */
  }, [open, initialTab]);
  ...
  const fetchShare = async () => {
    ...
    try {
      ...
      // 从 URL 中提取尾缀 (格式: /s/{suffix})
      const urlPath = data.url || "";
      const suffixMatch = urlPath.match(/\/s\/(.+)$/);
      if (suffixMatch) {
        setCustomSuffix(suffixMatch[1]);
      }
      setManageExpiresIn("0");
      setManageMaxDownloads(
        typeof data.maxDownloads === "number" ? String(data.maxDownloads) : ""
      );
      setDownloadCount(Number(data.downloadCount || 0));
      setMaxDownloads(
        typeof data.maxDownloads === "number" ? data.maxDownloads : null
      );
        ...
        {!shareToken ? (
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="bg-muted rounded p-2">
                可访问：
                {typeof maxDownloads === "number" ? maxDownloads : "不限"}
              </div>
              <div className="bg-muted rounded p-2">
                剩余：
                {typeof maxDownloads === "number"
                  ? Math.max(0, (maxDownloads || 0) - (downloadCount || 0))
                  : "-"}
              </div>
              <div className="bg-muted rounded p-2">
                有效期：
              ...
              <div className="bg-muted rounded p-2 col-span-2">
                口令：
                {passwordPlain ? (
                  <span className="font-mono">{passwordPlain}</span>
                ) : (
                  "未设置"
                )}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                ...
                <select
                  ...
                  value={manageExpiresIn}
                  onChange={(e) => setManageExpiresIn(e.target.value)}
                >
              ...
              <div>
                ...
                <Input
                  type="number"
                  placeholder="可选"
                  value={manageMaxDownloads}
                  onChange={(e) => setManageMaxDownloads(e.target.value)}
                  min={1}
                />
                <div>
                  <label className="text-sm font-medium mb-1 block">
                ...
                onClick={async () => {
                  try {
                    ...
                    if (customSuffix.trim())
                      payload.customSuffix = customSuffix.trim();
                    const res = await authFetch(
                      `/api/clipboard/${itemId}/share`,
                      {
                        method: "PUT",
              ...
              <Button
                onClick={async () => {
                  try {
                    ...
                    if (manageMaxDownloads.trim())
                      payload.maxDownloads = Number(manageMaxDownloads);
                    if (sharePassword.trim())
                      payload.password = sharePassword.trim();
                    // 添加自定义尾缀支持
                    if (customSuffix.trim())
