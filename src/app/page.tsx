...
import {
  CLIPBOARD_CREATED_EVENT,
  CLIPBOARD_DELETED_EVENT,
  CLIPBOARD_REORDERED_EVENT,
} from "@/lib/socket-events";
import type { ClipboardItem as GridItem } from "@/components/clipboard/ClipboardGrid";

const ClipboardGrid = dynamic(
  () => import("@/components/clipboard/ClipboardGrid"),
  { ssr: false }
);
const ClipboardList = dynamic(
  () => import("@/components/clipboard/ClipboardList"),
  { ssr: false }
);
const AddItemDialog = dynamic(
  () => import("@/components/clipboard/AddItemDialog"),
  { ssr: false }
);
const ItemDetailDialog = dynamic(
  () => import("@/components/clipboard/ItemDetailDialog"),
  { ssr: false }
);
// ShareManagerDialog no longer needed in new flow
const CreateShareDialog = dynamic(
  () => import("@/components/clipboard/CreateShareDialog"),
  { ssr: false }
);
...
export default function Home() {
  ...
  const [shareInitial, setShareInitial] = useState<{
    token: string;
    url: string;
  } | null>(null);
  const [shareInitialTab, setShareInitialTab] = useState<"status" | "settings">(
    "status"
  );
  const [qrOpen, setQrOpen] = useState(false);
  const [qrShare, setQrShare] = useState<{ token: string; url: string } | null>(
    null
  );
    ...
    try {
      const saved =
        typeof window !== "undefined"
          ? localStorage.getItem("clipboard_view_mode")
          : null;
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      if (typeof nextCursor.sortWeight === "number")
        params.set("cursorSortWeight", String(nextCursor.sortWeight));
  ...
  useEffect(() => {
    ...
    try {
      ...
      const useQueryAuth =
        (process.env.NEXT_PUBLIC_SSE_AUTH_IN_QUERY || "") === "1";
      const pwd = getStoredPassword();
      const url =
        useQueryAuth && pwd
          ? `${API_BASE}/api/events?auth=${encodeURIComponent(pwd)}`
          : `${API_BASE}/api/events`;
      ...
      es.addEventListener(CLIPBOARD_REORDERED_EVENT, (ev: MessageEvent) => {
        ...
        setItems((prev) => {
          ...
          if (hasWeights) {
            ...
            a = a.map((it) =>
              weights && Object.prototype.hasOwnProperty.call(weights, it.id)
                ? { ...it, sortWeight: weights![it.id] }
                : it
            );
            a.sort((x, y) => {
          ...
          a.sort((x, y) => {
            const ix = idIndex.has(x.id)
              ? idIndex.get(x.id)!
              : Number.POSITIVE_INFINITY;
            const iy = idIndex.has(y.id)
              ? idIndex.get(y.id)!
              : Number.POSITIVE_INFINITY;
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
            ...
            <div className="flex gap-2 items-center">
              <AddItemDialog
                ...
                onShareCreated={(share) => {
                  setQrShare({ token: share.token, url: share.url });
                  setQrOpen(true);
                }}
              />
              {/* 分享管理已整合到详情页，入口暂时隐藏 */}
        ...
        {viewMode === "grid" ? (
          <ClipboardGrid
            ...
            onRequestShare={(id: string) => {
              setShareItemId(id);
              setShareInitialTab("settings");
              setShareInitial(null);
              setShareOpen(true);
            }}
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          {qrShare ? (
            <div className="flex flex-col items-center gap-3">
              <img
                src={`/api/share/${qrShare.token}/qr?size=320`}
                alt="二维码"
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() =>
                    window.open(
                      `/api/share/${qrShare.token}/qr?size=1024&download=1`,
                      "_blank"
                    )
                  }
                >
                ...
                <Button
                  variant="outline"
                  onClick={async () => {
                    const ok = await safeCopyText(qrShare.url);
                    toast({
                      title: ok ? "已复制链接" : "请手动复制",
      ...
      <ItemDetailDialog
        ...
        onRequestEdit={async (id: string, content: string) => {
          try {
            const res = await authFetch(`/api/clipboard/${id}`, {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
        ...
        <div className="px-4 py-2 space-y-2">
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <div>
              <Select
                value={viewMode}
                onValueChange={(v) => onChangeViewMode(v as "grid" | "list")}
              >
